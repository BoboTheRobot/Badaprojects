...
FStroski::FStroski(int autoselectid):__pOptionMenu(null)
{
	this->autoselectid = autoselectid;
}

FStroski::~FStroski(void)
{
}
...
FStroski::Initialize()
{
	Form::Construct(L"IDF_FSTROSKI");

	return true;
}

result
FStroski::OnInitializing(void)
{
	result r = E_SUCCESS;
	...
	__pOptionMenu->AddItem("Categories", ID_EXPENSESKAT);
	__pOptionMenu->AddItem("Analytics", ID_EXPENSESPREGLED);
...
class CustomListElementStroski : public ICustomListElement
 {
public:
...
void
FStroski::AddListItem(Osp::Base::String col1, Osp::Base::String col2, Osp::Base::String col3, int itemId)
{

 CustomListItem * newItem = new CustomListItem();
 CustomListElementStroski * custom_element = new CustomListElementStroski();
...
void
FStroski::AddListItemSearch(Osp::Base::String col1, Osp::Base::String col2, Osp::Base::String col3, Osp::Base::String col4, int itemId, int searchposcol1, int searchposcol2)
{
...
void
FStroski::OnActionPerformed(const Control& source, int actionId)
{
	...
	if ((actionId == ID_EDIT) || (actionId == ID_DETAIL)) {
		if (this->selecteditemid > 0) {
			...
			if (actionId == ID_EDIT) {
				pFormMgr->SendUserEvent(FormMgr::REQUEST_EXPENSEADDEDIT, list);
			} else {
				pFormMgr->SendUserEvent(FormMgr::REQUEST_DETAILEXPENSE, list);
			}
		} else {
			MessageBox msgbox;
			int modalResult = 0;
		...
		if (this->selecteditemid > 0) {
			MessageBox msgbox;
			int modalResult = 0;
			if (carconclass_->DeleteExpenseData(this->selecteditemid, true) == false) {
				msgbox.Construct("Can't delete expense!", "Expense is selected in reference table!", MSGBOX_STYLE_OK, 10000);
				msgbox.ShowAndWait(modalResult);
			} else {
				msgbox.Construct("Delete expense?", "Do you really want to delete expense?", MSGBOX_STYLE_YESNO, 10000);
				msgbox.ShowAndWait(modalResult);
				if (modalResult == MSGBOX_RESULT_YES) {
					...
					pPopup_->Show();
					carconclass_->DeleteExpenseData(this->selecteditemid);
					int selecteditemindex = pExList->GetItemIndexFromItemId(this->selecteditemid);
					if (selecteditemindex > -1) {
		...
		FormMgr *pFormMgr = static_cast<FormMgr *>(pFrame->GetControl("FormMgr"));
		pFormMgr->SendUserEvent(FormMgr::REQUEST_EXPENSEADDEDIT, null);
	...
	} else if (actionId == ID_EXPENSESKAT) {
		FormMgr *pFormMgr = static_cast<FormMgr *>(pFrame->GetControl("FormMgr"));
		pFormMgr->SendUserEvent(FormMgr::REQUEST_EXPENSEKATFORM, null);
	} else if (actionId == ID_EXPENSESPREGLED) {
		FormMgr *pFormMgr = static_cast<FormMgr *>(pFrame->GetControl("FormMgr"));
...
void FStroski::OnItemStateChanged(const Osp::Ui::Control& source, int index, int itemId, Osp::Ui::ItemStatus status) {
	//
}

void FStroski::OnItemStateChanged(const Osp::Ui::Control& source, int index, int itemId, int elementId, Osp::Ui::ItemStatus status) {
...
void
FStroski::OnTouchDoublePressed(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{

}

void
FStroski::OnTouchFocusIn(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{
    //
}

void
FStroski::OnTouchFocusOut(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{
    //
}

void
FStroski::OnTouchLongPressed(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{
	if(&source==pExList) {
...
void
FStroski::OnTouchMoved(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{

}

void
FStroski::OnTouchPressed(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{
	if (&source == this) {
...
void
FStroski::OnTouchReleased(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{

}

void FStroski::OnTextValueChanged(const Osp::Ui::Control& source) {
	SearchAction(pKeypad->GetText());
}

void FStroski::OnTextValueChangeCanceled(const Osp::Ui::Control& source) {
