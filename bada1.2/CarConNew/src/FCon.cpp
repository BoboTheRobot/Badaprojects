...
FCon::FCon(int autoselectid):__pOptionMenu(null)
{
...
FCon::Initialize()
{
	Form::Construct(L"IDF_FCON");

	return true;
}

result
FCon::OnInitializing(void)
{
	result r = E_SUCCESS;
	...
	__pOptionMenu->AddItem("Analytics", ID_CONPREGLED);
	...
	CarConData data_;
	Osp::Base::String col2str, col3str;
	if (carconclass_->GetCarConDataListStart(carconclass_->SelectedCar.ID)) {
		while (carconclass_->GetCarConDataListGetData(data_)) {
			col2str = controlhandler_->DateFormater(data_.time, true);
			if (data_.Location > 0)
			col2str.Append(L", " + data_.LocationCaption);
			col3str = L"";
			if (data_.Type == 1) {
			if ((settingsdata_.contype == SETTINGSCONTYPE_LKM) || (settingsdata_.contype == SETTINGSCONTYPE_GMI) || (settingsdata_.contype == SETTINGSCONTYPE_KMG) || (settingsdata_.contype == SETTINGSCONTYPE_MIL)) {
				col3str.Format(25,L"%4.2f",data_.Con);
			} else {
				if (data_.Con > 0) {
					col3str.Format(25,L"%4.2f",(100 / data_.Con));
				} else {
					col3str.Format(25,L"%4.2f",0);
				}
			}
			} else {
			col3str.Append(L"N/A");
			}
			AddListItem(controlhandler_->CurrencyFormater(data_.Price * data_.StL), col2str, col3str, data_.ID);
		}
...
void FCon::SearchAction(Osp::Base::String searchqstr) {
		...
		if (carconclass_->GetCarConDataListStart(carconclass_->SelectedCar.ID)) {
			while (carconclass_->GetCarConDataListGetData(data_)) {
				...
				if (this->searchq.GetLength() > 0) {
					...
					lowercased2 = L"";
					if (data_.Location > 0) {
						data_.LocationCaption.ToLower(lowercased1);
					} else {
						data_.LocationCaption = L"Unknown location";
						lowercased1 = L"UNKL";
					}
					...
					if ((lowercased1.IndexOf(lowercasedsearchq,0,searchwfoundi1) == E_SUCCESS) || (lowercased2.IndexOf(lowercasedsearchq,0,searchwfoundi2) == E_SUCCESS)) {
						...
						if (isresult) {
							col2str = controlhandler_->DateFormater(data_.time, true);
							col2str.Append(L", " + controlhandler_->CurrencyFormater(data_.Price * data_.StL));
							AddListItemSearch(data_.LocationCaption, col2str, col3str, data_.Remark, data_.ID, searchwfoundi1, searchwfoundi2);
						}
					}
				} else {
					col2str = controlhandler_->DateFormater(data_.time, true);
...
class CustomListElementCon : public ICustomListElement
 {
public:
 Osp::Base::String col1, col2, col3, col4, me, searchq, col1b, col1s, col1a, col4b, col4s, col4a;
 result DrawElement(const Osp::Graphics::Canvas& canvas, const Osp::Graphics::Rectangle& rect, CustomListItemStatus itemStatus)
 {
     ...
     Font fontme_;
     fontme_.Construct(Osp::Graphics::FONT_STYLE_PLAIN, 24);
     ...
     texteelcol3.SetTextWrapStyle(TEXT_WRAP_CHARACTER_WRAP);
     TextElement textelcol3;
     textelcol3.Construct(col3 + L"\n");
     textelcol3.SetTextColor(Color::COLOR_WHITE);
     textelcol3.SetFont(fontcol3);
     texteelcol3.Add(textelcol3);
     TextElement textelme;
     textelme.Construct(me);
     textelme.SetTextColor(Color(230,230,230));
     textelme.SetFont(fontme_);
     texteelcol3.Add(textelme);
...
void
FCon::AddListItem(Osp::Base::String col1, Osp::Base::String col2, Osp::Base::String col3, int itemId)
{
 CustomListItem * newItem = new CustomListItem();
 CustomListElementCon * custom_element = new CustomListElementCon();
 ...
 custom_element->me = settingsdata_.contypestr;
...
void
FCon::AddListItemSearch(Osp::Base::String col1, Osp::Base::String col2, Osp::Base::String col3, Osp::Base::String col4, int itemId, int searchposcol1, int searchposcol2)
{
...
void
FCon::OnActionPerformed(const Control& source, int actionId)
{
	...
	if ((actionId == ID_EDIT) || (actionId == ID_DETAIL)) {
		if (this->selecteditemid > 0) {
			...
			if (actionId == ID_EDIT) {
				pFormMgr->SendUserEvent(FormMgr::REQUEST_ADDEDITPORABA, list);
			} else {
				pFormMgr->SendUserEvent(FormMgr::REQUEST_DETAILPORABE, list);
			}
		} else {
			MessageBox msgbox;
			int modalResult = 0;
		...
		if (this->selecteditemid > 0) {
			MessageBox msgbox;
			int modalResult = 0;
			if (carconclass_->DeleteCarConData(this->selecteditemid, true) == false) {
				msgbox.Construct("Can't delete record!", "Error!", MSGBOX_STYLE_OK, 10000);
				msgbox.ShowAndWait(modalResult);
			} else {
				msgbox.Construct("Delete fill up record?", "Do you really want to delete fill up record?", MSGBOX_STYLE_YESNO, 10000);
				msgbox.ShowAndWait(modalResult);
				if (modalResult == MSGBOX_RESULT_YES) {
					...
					pPopup_->Show();
					carconclass_->DeleteCarConData(this->selecteditemid);
					this->searchq = this->searchq + "!!!";
					SearchAction(L"");
					this->selecteditemid = 0;
		...
		FormMgr *pFormMgr = static_cast<FormMgr *>(pFrame->GetControl("FormMgr"));
		pFormMgr->SendUserEvent(FormMgr::REQUEST_ADDEDITPORABA, null);
	} else if (actionId == ID_BACK) {
		FormMgr *pFormMgr = static_cast<FormMgr *>(pFrame->GetControl("FormMgr"));
		pFormMgr->SendUserEvent(FormMgr::REQUEST_MAINFORM, null);
	} else if (actionId == ID_OPTIONKEY) {
		if (__pOptionMenu != null) {
			__pOptionMenu->SetShowState(true);
			__pOptionMenu->Show();
		}
	} else if (actionId == ID_CONPREGLED) {
		FormMgr *pFormMgr = static_cast<FormMgr *>(pFrame->GetControl("FormMgr"));
...
void FCon::OnItemStateChanged(const Osp::Ui::Control& source, int index, int itemId, Osp::Ui::ItemStatus status) {
	//
}

void FCon::OnItemStateChanged(const Osp::Ui::Control& source, int index, int itemId, int elementId, Osp::Ui::ItemStatus status) {
...
void
FCon::OnTouchDoublePressed(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{

}

void
FCon::OnTouchFocusIn(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{
    //
}

void
FCon::OnTouchFocusOut(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{
    //
}

void
FCon::OnTouchLongPressed(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{
	if(&source==pExList) {
...
void
FCon::OnTouchMoved(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{

}

void
FCon::OnTouchPressed(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{
	if (&source == this) {
...
void
FCon::OnTouchReleased(const Control &source, const Point &currentPosition, const TouchEventInfo &touchInfo)
{

}

void FCon::OnTextValueChanged(const Osp::Ui::Control& source) {
	SearchAction(pKeypad->GetText());
}

void FCon::OnTextValueChangeCanceled(const Osp::Ui::Control& source) {
